...
struct ComicListInteractorTest: ComicsInteractorProtocol {
    ...
    func fetchComics(page: Int) async throws -> [ComicModel] {
        ...
        guard let url = Bundle.main.url(forResource: "ComicTestPaginated", withExtension: "json") else { return [] } //creamos la url de dipositivo
        let data = try Data(contentsOf: url) // cargamos los datos
        
        return try JSONDecoder().decode(ComicDataDTO.self, from: data).items.map(\.mapToComicModel) //Decodificamos los datos
        
    }
}

//    func searchOnStore(text: String) async throws -> ComicDataDTO {
//        let url = URL.comicSearchContainsURL.appending(path: text)
//        print(url)
//        var components = URLComponents(url: url, resolvingAgainstBaseURL: false)!
//        components.queryItems = [
//            URLQueryItem(name: "name", value: "dragon")
//        ]
